...
public class AddDecodeNodeForDictStringRule implements PhysicalOperatorTreeRewriteRule {
    ...
    public OptExpression rewrite(OptExpression root, TaskContext taskContext) {
        ...
        if (context.hasChanged) {
            ...
            OptExpression result = OptExpression.create(decodeOperator, rewriteExpr);
            result.setStatistics(rewriteExpr.getStatistics());
            result.setLogicalProperty(rewriteExpr.getLogicalProperty());
            return result;
        }
        return rewriteExpr;
    }
    ...
    public static void insertDecodeExpr(OptExpression parentExpr, List<OptExpression> childExpr,
        ...
        ImmutableMap<Integer, Integer> dictIdsToStringIds = builder.build();
        PhysicalDecodeOperator decodeOperator = new PhysicalDecodeOperator(dictIdsToStringIds);
        OptExpression decodeExp = OptExpression.create(decodeOperator, childExpr);
        decodeExp.setStatistics(childExpr.get(0).getStatistics());
        decodeExp.setLogicalProperty(childExpr.get(0).getLogicalProperty());
